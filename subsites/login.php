...
if (isset($_GET["logout"])) {
    ...
    session_destroy();
    $_SESSION = array();
    echo '<script>window.href.location="?login=logout"</script>';
    ...
    if (mysqli_num_rows($login) == 1) {
        ...
        echo '<script>window.href.location="?login=sucess"</script>';
    } else echo '<script>window.href.location="?login=failure"</script>';
} else {?>
