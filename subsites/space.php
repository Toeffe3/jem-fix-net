<?php
    ...
    if(isset($_GET["update"])) {
        ...
        foreach($post as $key => $v)
            if(!mysqli_query($conn, "UPDATE `spaces` SET `x`='".$v["x"]."', `y`='".$v["y"]."', `w`='".$v["w"]."', `h`='".$v["h"]."' WHERE `stoargeid` = '".$key."'")) {
                echo false; exit;
			}
        echo true;
        exit;
        ...
        <table id="cyklisk">
            ...
            <?php
                ...
                while($space = mysqli_fetch_assoc($spaces)) {
                    echo '<tr><th'.(isset($_GET["id"]) && $space["stoargeid"]==$_GET["id"]?" class='white bg-dark bg-red'":"").'><a href="?space&id='.$space["stoargeid"].'">'.$space["stoargeid"].'</a></th><th>'.$space["spaceid"].'</th></tr>';
                }
            ?>
        ...
        <script>
            ...
            map.onmouseup = function() {
                if(document.selSpace) {
                    ...
                    for(space of spaces)
                        newpos[space.id] = {"x": space.x.baseVal.value, "y": space.y.baseVal.value, "w": space.width.baseVal.value<20?20:space.width.baseVal.value, "h": space.height.baseVal.value<10?20:space.height.baseVal.value};
